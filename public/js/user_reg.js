...
//注册邮箱提示
function email_tips(){
	email_msg.style.color="#007bff";
	email_msg.innerHTML="*例如QQ邮箱,网易邮箱"
}
//注册邮箱验证
...
//注册真实姓名提示
function user_name_tips(){
	user_name_msg.style.color="#007bff";
	user_name_msg.innerHTML="*请输入你的真实姓名"
}
//注册真实姓名验证
function user_name_check(){
	var t=/^[\u4E00-\u9FA5A-Za-]+$/;
	if(user_name.value==""){
  	user_name_msg.style.color="#dc3545";
		user_name_msg.innerHTML="*真实姓名必须填写";
		user_name_pass=0;
	}else if(t.test(user_name.value)){
		user_name_msg.style.color="#28a745";
		user_name_msg.innerHTML="真实姓名合法";
		user_name_pass=1;
	}else{
  	user_name_msg.style.color="#dc3545";
		user_name_msg.innerHTML="*请输入合法的姓名";
		user_name_pass=0;
	}
}
//注册按钮验证
function login_check(){
	...
	var xhr=new XMLHttpRequest();
	var url=`/user/reg?uname=${uname.value}&upwd=${upwd.value}&email=${email.value}&phone=${phone.value}&user_name=${user_name.value}&sex=${sex}`;
	xhr.open("get",url,true);
	xhr.send(null);
	xhr.onreadystatechange=function(){
		if(xhr.readyState==4 && xhr.status==200){
